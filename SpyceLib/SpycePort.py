...
class Port(QtWidgets.QGraphicsPathItem):
    ...
    def __init__(self, parent, scene, label=None, porttype='node', flip=False):
        ...
        self.setLabel()
        self.setFlip()
        if self not in list(scene.items()): # added since pins/nodes were not auto added in Qt5? 
            scene.addItem(self)
    ...
    def setFlip(self):
        if isPort(self, 'block'): # flip is done via parent (only label needs processing)
